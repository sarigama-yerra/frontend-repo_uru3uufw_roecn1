...
export default function Deals({ title = 'Top Deals for You', deals = defaultDeals }) {
  return (
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {deals.map((d, idx) => (
            <a key={idx} href="#" className="group rounded-xl overflow-hidden bg-white border border-slate-200 hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                <h3 className="font-medium text-slate-800 group-hover:text-slate-900 line-clamp-1">{d.title}</h3>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-slate-900 font-semibold">${d.price.toFixed(2)}</span>
                  <span className="text-[11px] px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">In stock</span>
                </div>
              </div>
            </a>
          ))}
